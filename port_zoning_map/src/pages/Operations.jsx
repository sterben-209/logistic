...
<section className="flex-[2] flex flex-col bg-surface-container/90 backdrop-blur-sm rounded-xl border border-outline-variant/30 shadow-lg overflow-hidden flex flex-col h-full">
...
{(() => {
  ...
  if (allContainers.length === 0) {
    return (
      <tr>
        <td colSpan="8" className="text-center py-8 text-outline-variant">Waiting for Map data...</td>
      </tr>
    );
  }
  
  return allContainers.map((container, index) => (
    <tr key={container.containerNo + index} className="data-grid-row">
      <td className="data-grid-cell text-center text-outline-variant">{index + 1}</td>
      <td className="data-grid-cell font-bold text-on-surface">{container.containerNo || container.id}</td>
      <td className="data-grid-cell text-on-surface-variant">{container.size}ft {container.type || container.cargoType}</td>
      <td className="data-grid-cell">{container.weight || 20}</td>
      ...
      {slotsData.length === 0 ? (
        ...
        slotsData.map((slot, index) => (
          <tr key={slot.id} className="border-b border-outline-variant/10 text-sm hover:bg-surface-container-highest transition-colors">
             <td className="py-2 text-center text-outline-variant">{index + 1}</td>
  ...
  <table className="w-full text-left border-collapse">
    <thead>
      <tr>
        ...
        <th className="data-grid-header">Progress</th>
      </tr>
    </thead>
    <tbody id="operations-fleet-tbody">
      {fleetData.length === 0 ? (
        <tr>
          <td colSpan="4" className="text-center py-8 text-outline-variant">Waiting for Live Map telemetry...</td>
        </tr>
      ) : (
        fleetData.map(v => (
          <tr key={v.id} className="border-b border-outline-variant/10 text-sm">
             <td className="py-2 text-primary font-bold">{v.truckPlate || v.id}</td>
             <td className="py-2 text-secondary">Tractor ({v.type})</td>
             <td className="py-2">
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${v.status === 'EN_ROUTE_TO_SLOT' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-500'}`}>
                  {v.status.replace(/_/g, ' ')}
                </span>
             </td>
             <td className="py-2">
                <div className="w-full bg-surface-container-highest h-2 rounded overflow-hidden">
                   <div className={`bg-primary h-full`} style={{ width: `${Math.round(v.progress || 0)}%` }}></div>
                </div>
             </td>
          </tr>
        ))
...
<section className="flex-1 flex flex-col bg-[#050914] rounded-xl border border-outline-variant/20 shadow-lg overflow-hidden h-full min-w-[320px]">
<div className="p-3 border-b border-outline-variant/20 bg-surface-container-lowest flex justify-between items-center">
<h3 className="font-label-sm text-sm text-outline flex items-center gap-2 tracking-wider">
...
<div className="w-2.5 h-2.5 rounded-full bg-surface-variant"></div>
<div className="w-2.5 h-2.5 rounded-full bg-secondary/50 animate-pulse"></div>
</div>
</div>
<div className="flex-1 p-4 overflow-y-auto terminal-text">
<div className="terminal-log-entry info">
<span className="text-outline-variant">[10:42:01]</span> <span className="text-primary-fixed-dim">SYS:</span> Connecting to master orchestrator...
                    </div>
<div className="terminal-log-entry success">
<span className="text-outline-variant">[10:42:02]</span> <span className="text-secondary-fixed">NET:</span> Connection established. Latency: 12ms
                    </div>
<div className="terminal-log-entry info">
<span className="text-outline-variant">[10:42:05]</span> <span className="text-primary-fixed-dim">SYNC:</span> Receiving yard layout state (v4.2.1)
                    </div>
<div className="terminal-log-entry info">
<span className="text-outline-variant">[10:42:08]</span> <span className="text-primary-fixed-dim">OP:</span> RTG-Crane-04 dispatched to zone A-14
                    </div>
<div className="terminal-log-entry info">
<span className="text-outline-variant">[10:42:15]</span> <span className="text-primary-fixed-dim">DATA:</span> Matrix updated. 8234 containers tracked.
                    </div>
<div className="terminal-log-entry warning">
<span className="text-outline-variant">[10:43:22]</span> <span className="text-tertiary">WARN:</span> Slight temp deviation detected in NX-9903 (Reefer). Target: -18C, Current: -17.2C
                    </div>
<div className="terminal-log-entry info">
<span className="text-outline-variant">[10:44:01]</span> <span className="text-primary-fixed-dim">OP:</span> AGV-Unit-99 en route to D-10. ETA 4m 12s.
                    </div>
<div className="terminal-log-entry success">
<span className="text-outline-variant">[10:45:30]</span> <span className="text-secondary-fixed">DATA:</span> Heartbeat ACK. System nominal.
                    </div>
<div className="terminal-log-entry info mt-4">
